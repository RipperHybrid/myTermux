...
| 5 | **Extra CLI Tools** | optional | Installs `ffmpeg`, `python`, `nodejs`, `yt-dlp`, and `gallery-dl` for the `ytdl` / `dlv` media downloader, plus keeps `gitssh`, `ipconfig`, `macfinder`, and the JS repo checker | Removes them plus the `repocek` / `convi` / `dlv` aliases |
