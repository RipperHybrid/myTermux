# macOS clean zsh theme with user & git support
...
PROMPT="%F{8}%K{8}%(?:%F{2}● %F{3}● %F{1}●:%F{1} %F{1}● ●)%{%k%}%F{8}%f "
PROMPT+="%F{8}%K{8}%F{3} $(mytermux_user)%{%k%}%F{8}%f "
PROMPT+="%F{8}%K{8}%F{6} %2~%{%k%}%F{8}%f "
PROMPT+='$(git_prompt_info) %(?.%F{2}❯%f.%F{1}❯%f) '
...
ZSH_THEME_GIT_PROMPT_PREFIX="%F{8}%K{8}%F{5} %F{7}"
