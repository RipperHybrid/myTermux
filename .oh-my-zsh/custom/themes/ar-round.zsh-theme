# Ar-Round minimal zsh theme with user & git support
...
if [[ $EUID -eq 0 ]]; then
    ...
    user_symbol="%(?.%F{5}%f.%F{1}%f)"
fi

user_pill="%F{8}%K{8}%F{5} $(mytermux_user) %{%k%}%F{8}%f"
dir_path="%F{8}%K{8}%F{4} %K{0}%F{7} %2~ %{%k%}%F{0}%f"
...
ZSH_THEME_GIT_PROMPT_PREFIX="%F{8}%K{8}%F{4} %K{0}%F{7} "
